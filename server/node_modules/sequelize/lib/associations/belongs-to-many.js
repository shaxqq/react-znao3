...
class BelongsToMany extends Association {
  ...
  count(instance, options) {
    ...
    return this.get(instance, options).then(result =>
  ...
  has(sourceInstance, instances, options) {
    ...
    return this.get(sourceInstance, options).then(
      associatedObjects =>
        _.differenceBy(instancePrimaryKeys, associatedObjects, this.targetKey)
          .length === 0
    );
  }
  ...
  add(sourceInstance, newInstances, options) {
    ...
    const updateAssociations = currentRows => {
      ...
      if (unassociatedObjects.length > 0) {
        ...
        promises.push(
          association.through.model.bulkCreate(
            bulk,
            Object.assign({ validate: true }, options)
          )
        );
  ...
  create(sourceInstance, values, options) {
    ...
    return association.target
      ...
      .then(newAssociatedObject =>
        sourceInstance[association.accessors.add](
          newAssociatedObject,
