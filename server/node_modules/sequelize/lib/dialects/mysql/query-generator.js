...
class MySQLQueryGenerator extends AbstractQueryGenerator {
  ...
  createDatabaseQuery(databaseName, options) {
    ...
    const charset = options.charset
      ...
      : "";
    const collate = options.collate
      ? ` DEFAULT COLLATE ${this.escape(options.collate)}`
      : "";
  ...
  dropDatabaseQuery(databaseName) {
    return `DROP DATABASE IF EXISTS ${this.quoteIdentifier(
      databaseName
    ).trim()};`;
  }
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    const rowFormat = options.rowFormat
      ? ` ROW_FORMAT=${options.rowFormat}`
      : "";
  ...
  removeColumnQuery(tableName, attributeName) {
    return `ALTER TABLE ${this.quoteTable(
      tableName
    )} DROP ${this.quoteIdentifier(attributeName)};`;
  }
  ...
  removeIndexQuery(tableName, indexNameOrAttributes) {
    ...
    return `DROP INDEX ${this.quoteIdentifier(indexName)} ON ${this.quoteTable(
      tableName
    )}`;
  }
  ...
  getForeignKeyQuery(table, columnName) {
    ...
    return (
      `SELECT ${foreignKeyFields} FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE` +
      ` WHERE (REFERENCED_TABLE_NAME = ${quotedTableName}${
        table.schema ? ` AND REFERENCED_TABLE_SCHEMA = ${quotedSchemaName}` : ""
      } AND REFERENCED_COLUMN_NAME = ${quotedColumnName})` +
      ` OR (TABLE_NAME = ${quotedTableName}${
        table.schema ? ` AND TABLE_SCHEMA = ${quotedSchemaName}` : ""
      } AND COLUMN_NAME = ${quotedColumnName} AND REFERENCED_TABLE_NAME IS NOT NULL)`
    );
  }
