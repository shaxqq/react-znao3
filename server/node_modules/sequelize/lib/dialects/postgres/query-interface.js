...
function ensureEnums(qi, tableName, attributes, options, model) {
  ...
  for (i = 0; i < keyLen; i++) {
    ...
    if (
      ...
      sql = qi.QueryGenerator.pgListEnums(
        tableName,
  ...
  return Promise.all(promises).then(results => {
    ...
    const addEnumValue = (
      ...
      promises.splice(spliceStart, 0, () => {
        return qi.sequelize.query(
          qi.QueryGenerator.pgEnumAdd(tableName, field, value, valueOptions),
          valueOptions
        );
      });
    };
    ...
    for (i = 0; i < keyLen; i++) {
      ...
      if (
        ...
        if (!results[enumIdx]) {
          promises.push(() => {
            return qi.sequelize.query(
              qi.QueryGenerator.pgEnum(tableName, field, enumType, options),
              Object.assign({}, options, { raw: true })
            );
          });
        } else if (!!results[enumIdx] && !!model) {
          const enumVals = qi.QueryGenerator.fromArray(
    ...
    return promises
      .reduce(
        (promise, asyncFunction) => promise.then(asyncFunction),
        Promise.resolve()
      )
      .tap(() => {
