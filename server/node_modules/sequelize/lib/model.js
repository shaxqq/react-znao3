...
class Model {
  ...
  static _validateIncludedElement(include, tableNames, options) {
    ...
    if (include.attributes && !options.raw) {
      ...
      include.originalAttributes = this._injectDependentVirtualAttributes(
        include.attributes
      );
      ...
      .then(() => {
        ...
        return Promise.all([
          ...
          _.each(attributes, (columnDesc, columnName) => {
            if (
              !columns[columnName] &&
              !columns[attributes[columnName].field]
            ) {
              changes.push(() =>
  ...
  static findAndCountAll(options) {
    ...
    return Promise.all([this.count(countOptions), this.findAll(options)]).then(
      ([count, rows]) => ({
        count,
        rows: count === 0 ? [] : rows
      })
  ...
  static findCreateFind(options) {
    ...
    return this.findOne(options).then(result => {
      ...
      return this.create(values, options)
        ...
        .catch(sequelizeErrors.UniqueConstraintError, () =>
    ...
    const recursiveBulkCreate = (instances, options) => {
      ...
      if (options.updateOnDuplicate !== undefined) {
        if (
          ...
          return Promise.reject(
            new Error("updateOnDuplicate option only supports non-empty array.")
          );
        }
      }
            ...
            .then(() => {
              ...
              return Promise.map(
                ...
                include => {
                  ...
                  return recursiveBulkCreate(
                    associationInstances,
                    includeOptions
                  ).then(associationInstances => {
                    for (const idx in associationInstances) {
                      const associationInstance = associationInstances[idx];
                      const instance =
                        associationInstanceIndexToInstanceMap[idx];

                      instance[include.association.accessors.set](
                        associationInstance,
                        { save: false, logging: options.logging }
                      );
                    }
                  });
                }
            ...
            .then(() => {
              ...
              records = instances.map(instance => {
                ...
                const out = Object.assign(
                  {},
                  Utils.mapValueFieldNames(values, options.fields, model)
                );
              ...
              if (options.returning && Array.isArray(options.returning)) {
                options.returning = options.returning.map(
                  attr => model.rawAttributes[attr].field || attr
                );
              }

              return model.QueryInterface.bulkInsert(
              ...
              return recursiveBulkCreate(
                ...
                if (include.association instanceof BelongsToMany) {
                  ...
                  for (const idx in associationInstances) {
                    ...
                    const values = {};
                    values[include.association.foreignKey] = instance.get(
                      instance.constructor.primaryKeyAttribute,
                      { raw: true }
                    );
                    values[
                      include.association.otherKey
                    ] = associationInstance.get(
    ...
    return Promise.try(() => {
      ...
      .then(() => {
        ...
        if (this.isNewRecord) {
          ...
          if (versionAttr) {
            values[versionFieldName] =
              parseInt(values[versionFieldName], 10) + 1;
          }
          query = "update";
          args = [
            this,
                    ...
                    return instance.save(includeOptions).then(() => {
                      ...
                      if (instance[include.association.through.model.name]) {
                        for (const attr of Object.keys(
                          ...
                          values[attr] =
                            instance[include.association.through.model.name][
                              attr
                            ];
                        }
                      }
  ...
  restore(options) {
    ...
    return Promise.try(() => {
      ...
      .then(() => {
        ...
        return this.save(
          Object.assign({}, options, { hooks: false, omitNull: false })
        );
      })
      .tap(() => {
        // Run after hook
