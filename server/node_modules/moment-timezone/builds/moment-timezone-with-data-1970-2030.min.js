!(function(c, M) {
  ...
  function o(c) {
    var M = 0,
      ...
      p = 0,
      a = 1;
    for (45 === c.charCodeAt(0) && (a = -(M = 1)); M < b.length; M++)
      p = 60 * p + n(b.charCodeAt(M));
    for (M = 0; M < o.length; M++) (z /= 60), (p += n(o.charCodeAt(M)) * z);
    return p * a;
  }
  function O(c) {
  ...
  function q(c, M) {
    var A,
      b = [];
    for (A = 0; A < M.length; A++) b[A] = c[M[A]];
    return b;
  }
  function r(c) {
    var M = c.split("|"),
  ...
  function L(c, M) {
    for (var A, b; (b = 6e4 * (((M.at - c.at) / 12e4) | 0)); )
      (A = new N(new Date(c.at + b))).offset === c.offset ? (c = A) : (M = A);
    return c;
  }
  function W(c, M) {
    ...
    var A,
      ...
      z = (function() {
        var c,
          ...
          o = new N(new Date(b, 0, 1)),
          z = [o];
        for (A = 1; A < 48; A++)
          (M = new N(new Date(b, A, 1))).offset !== o.offset &&
            ((c = L(o, M)), z.push(c), z.push(new N(new Date(c.at + 6e4)))),
            (o = M);
        for (A = 0; A < 4; A++)
          z.push(new N(new Date(b + A, 0, 1))),
            z.push(new N(new Date(b + A, 6, 1)));
        return z;
      })(),
      p = z.length,
      a = (function(c) {
        var M,
          ...
          p = [];
        for (M = 0; M < o; M++)
          for (A in (b = e[c[M].offset] || {}))
            b.hasOwnProperty(A) && (z[A] = !0);
        for (M in z) z.hasOwnProperty(M) && p.push(i[M]);
        return p;
      })(z),
      n = [];
    for (b = 0; b < a.length; b++) {
      for (A = new f(u(a[b]), p), o = 0; o < p; o++) A.scoreOffsetAt(z[o]);
      n.push(A);
    }
    return n.sort(W), 0 < n.length ? n[0].zone.name : void 0;
  }
  function l(c) {
  ...
  function u(c, M) {
    ...
    return b instanceof d
      ...
      ? ((b = new d(b)), (p[c] = b))
      : a[c] && M !== u && (A = u(a[c], u))
      ? ((b = p[c] = new d())._set(A), (b.name = i[c]), b)
      : null;
  }
  function s(c) {
    var M, A, b, o;
  ...
  function T(c) {
    t(c.zones), s(c.links), (E.dataVersion = c.version);
  }
  function m(c) {
    var M = "X" === c._f || "x" === c._f;
  ...
  function E(c) {
    var M = Array.prototype.slice.call(arguments, 0, -1),
      ...
      b = u(A),
      o = z.utc.apply(null, M);
    return (
      b && !z.isMoment(c) && m(o) && o.add(b.parse(o), "minutes"), o.tz(A), o
    );
  }
  (A < 2 || (2 == A && b < 6)) &&
    R(
    ...
    (d.prototype = {
      ...
      _index: function(c) {
        var M,
          A = +c,
          b = this.untils;
        for (M = 0; M < b.length; M++) if (A < b[M]) return M;
      },
      parse: function(c) {
        var M,
          A,
        ...
        for (o = 0; o < n; o++)
          if (
            ...
            (A = p[o + 1]),
            (b = p[o ? o - 1 : o]),
            M < A && E.moveAmbiguousForward
              ? (M = A)
              : b < M && E.moveInvalidForward && (M = b),
            z < a[o] - 6e4 * M)
          )
            return p[o];
        return p[n];
    ...
    (E.names = function() {
      var c,
        M = [];
      for (c in i)
        i.hasOwnProperty(c) && (p[c] || p[a[c]]) && i[c] && M.push(i[c]);
      return M.sort();
    }),
    (E.Zone = d),
    (E.unpack = r),
  ...
  (z.tz = E),
      ...
      if (
        (void 0 === c._z &&
          (b &&
            m(c) &&
            !c._isUTC &&
            ((c._d = z.utc(c._a)._d), c.utc().add(b.parse(c), "minutes")),
          (c._z = b)),
        c._z)
      )
        if (
    ...
    (C.zoneName = h(C.zoneName)),
    (C.zoneAbbr = h(C.zoneAbbr)),
    (C.utc = D(C.utc)),
    (C.local = D(C.local)),
    (C.utcOffset =
      ((S = C.utcOffset),
      function() {
        return (
          0 < arguments.length && (this._z = null), S.apply(this, arguments)
        );
      })),
    (z.tz.setDefault = function(c) {
      return (
        ...
        (z.defaultZone = c ? u(c) : null),
        z
      );
    });
  var g = z.momentProperties;
  return (
