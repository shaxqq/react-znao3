...
const removeColumn = function(qi, tableName, attributeName, options) {
  ...
  const findConstraintSql = qi.QueryGenerator.getDefaultConstraintQuery(
    tableName,
    attributeName
  );
  return qi.sequelize
    ...
    .then(([results]) => {
      ...
      const dropConstraintSql = qi.QueryGenerator.dropConstraintQuery(
        tableName,
        results[0].name
      );
    ...
    .then(() => {
      const findForeignKeySql = qi.QueryGenerator.getForeignKeyQuery(
        tableName,
        attributeName
      );
    ...
    .then(([results]) => {
      ...
      const dropForeignKeySql = qi.QueryGenerator.dropForeignKeyQuery(
        tableName,
    ...
    .then(() => {
      ...
      const primaryKeyConstraintSql = qi.QueryGenerator.getPrimaryKeyConstraintQuery(
        tableName,
        attributeName
      );
    ...
    .then(() => {
      const removeSql = qi.QueryGenerator.removeColumnQuery(
        tableName,
        attributeName
      );
