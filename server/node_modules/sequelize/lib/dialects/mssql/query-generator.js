...
class MSSQLQueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    const query = (table, attrs) =>
        `IF OBJECT_ID('${table}', 'U') IS NULL CREATE TABLE ${table} (${attrs})`,
      primaryKeys = [],
      foreignKeys = {},
      attrStr = [];
  ...
  removeColumnQuery(tableName, attributeName) {
    return `ALTER TABLE ${this.quoteTable(
      tableName
    )} DROP COLUMN ${this.quoteIdentifier(attributeName)};`;
  }
  ...
  changeColumnQuery(tableName, attributes) {
    ...
    return `ALTER TABLE ${this.quoteTable(
      tableName
    )} ${finalQuery};${commentString}`;
  }

  renameColumnQuery(tableName, attrBefore, attributes) {
    const newName = Object.keys(attributes)[0];
    return `EXEC sp_rename '${this.quoteTable(
      tableName
    )}.${attrBefore}', '${newName}', 'COLUMN';`;
  }
  ...
  getForeignKeyQuery(table, attributeName) {
    const tableName = table.tableName || table;
    let sql = `${this._getForeignKeysQueryPrefix()} WHERE TB.NAME =${wrapSingleQuote(
      tableName
  ...
  dropForeignKeyQuery(tableName, foreignKey) {
    return `ALTER TABLE ${this.quoteTable(
      tableName
    )} DROP ${this.quoteIdentifier(foreignKey)}`;
  }
    ...
    if (
      ...
      if (options.offset) {
        ...
        const fragment =
          ...
          `(SELECT ${topFragment}*` +
          ` FROM (SELECT ROW_NUMBER() OVER (ORDER BY ${orders.mainQueryOrder.join(
            ", "
          )}) as row_num, * ` +
      ...
      mainFragment = `SELECT ${topFragment}${attributes.join(
        ", "
      )} FROM ${tables}`;
    }
