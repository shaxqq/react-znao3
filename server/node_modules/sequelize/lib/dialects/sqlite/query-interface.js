...
function removeColumn(qi, tableName, attributeName, options) {
  ...
  return qi.describeTable(tableName, options).then(fields => {
    ...
    return Promise.each(subQueries, subQuery =>
      qi.sequelize.query(`${subQuery};`, Object.assign({ raw: true }, options))
    );
  });
}
...
function renameColumn(qi, tableName, attrNameBefore, attrNameAfter, options) {
  ...
  return qi.describeTable(tableName, options).then(fields => {
    ...
    const sql = qi.QueryGenerator.renameColumnQuery(
      tableName,
      attrNameBefore,
      attrNameAfter,
      fields
...
function removeConstraint(qi, tableName, constraintName, options) {
  ...
  return qi
    .showConstraint(tableName, constraintName)
    .then(constraints => {
      ...
      if (constraint) {
        ...
        constraint.constraintName = qi.QueryGenerator.quoteIdentifier(
          constraint.constraintName
        );
        let constraintSnippet = `, CONSTRAINT ${constraint.constraintName} ${constraint.constraintType} ${constraint.constraintCondition}`;

        if (constraint.constraintType === "FOREIGN KEY") {
          const referenceTableName = qi.QueryGenerator.quoteTable(
            constraint.referenceTableName
          );
          constraint.referenceTableKeys = constraint.referenceTableKeys.map(
            columnName => qi.QueryGenerator.quoteIdentifier(columnName)
          );
    ...
    .then(fields => {
      const sql = qi.QueryGenerator._alterConstraintQuery(
        tableName,
        fields,
        createTableSql
      );
...
function addConstraint(qi, tableName, options) {
  const constraintSnippet = qi.QueryGenerator.getConstraintSnippet(
    tableName,
    options
  );
  const describeCreateTableSql = qi.QueryGenerator.describeCreateTableQuery(
    tableName
  );
  let createTableSql;
  ...
  return qi.sequelize
    .query(
      ...
      Object.assign({}, options, { type: QueryTypes.SELECT, raw: true })
    )
    .then(constraints => {
